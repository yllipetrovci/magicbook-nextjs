...
export const OrderSummary: React.FC<OrderSummaryProps> = ({
    ...
    const pricePerStory = typeof credits === 'number' && credits > 0
        ? (priceDataInNumbers.total / credits).toFixed(2)
        : '0.00';
    ...
    return (
        <div className="flex-1 bg-magic-card p-8 rounded-3xl shadow-xl border border-white/5 h-fit flex flex-col">
            ...
            <div className="border-t border-white/10 py-4 space-y-2">
                <div className="flex justify-between text-gray-400 text-sm"><span>Subtotal</span><span>{price}</span></div>
                {priceDataInNumbers?.discount > 0 && (
                <div className="flex justify-between text-green-400 font-bold text-sm"><span>Coupon (MAGICFUN)</span><span>{discount}</span></div>
                 )} 
                <div className="flex justify-between text-gray-400 text-sm"><span>tax</span><span>{tax}</span></div>
            </div>
